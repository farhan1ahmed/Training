...
def confirm_user(token, status):
    ...
    if status == status_cancel:
        ...
        db.session.commit()
        return Response('{"message":"User deleted successfully"}', status=status_codes.NOT_FOUND, mimetype='application/json')
    elif status == status_confirm:
        if user.confirmed:
            return Response('{"message":"User already confirmed"}', status=status_codes.OK, mimetype='application/json')
        else:
            user.confirmed = True
            db.session.add(user)
            db.session.commit()
